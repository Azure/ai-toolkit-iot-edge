	...
	public partial class ViewController : UIViewController, IAVCaptureVideoDataOutputSampleBufferDelegate
	{
		...
		public VNRequest[] ClassificationRequest
		{
			get
			{
				if (model == null)
				{
                    ...
                    var modelPath = NSBundle.MainBundle.GetUrlForResource("CVS_SkinCancer_iter1", "mlmodelc");
		...
		public void handleClassification(VNRequest request, NSError error)
		{
			...
			else
			{
				...
				DispatchQueue.MainQueue.DispatchAsync(() =>
				{
                    var confidenceString = best.Confidence.ToString("P0", new NumberFormatInfo { PercentPositivePattern = 1 });
                    if (className == "benign")
                    {
